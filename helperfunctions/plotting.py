...
#%%
def adjust_figsize(fhandle,height=None,width=None,dpi=100,margins=None):
    '''
    adjust_figsize(fhandle,height=123)
    
    Adjusts the size of a figure with a single subplot for saving it in a well-defined
    size and format for publications etc. Sizes are to be given in mm. If either widht
    or height are None, the aspect ratio will be maintained.
    
    Optional inputs:
        * dpi: resolution for later export to file
        * margins: spacing between main plot and figure border
            * if None, figure will be cropped to main plot w/o ticks etc
            * if 'auto', the default values will be used
            * can be specified as [left,right,top,bottom], each with values 0-1
    
    Note: dpi will only be used to compute the border width; please provide the DPI that
    you want to use for saving the figure. The DPI of the plot will not be changed here 
    as large DPIs would not fit on the screen.
    '''
    plt.pause(1) # to give time to render
    # get old size of figure & axis
    ahandle = fhandle.gca()
    if(height is None or width is None): ahandle.set_aspect('equal')
    bbox = ahandle.get_window_extent().transformed(fhandle.dpi_scale_trans.inverted())
    hao = bbox.height * 25.4
    wao = bbox.width  * 25.4
    # determine new size of figure & axis
    hn = height if(height is not None) else hao * (width  / wao);
    wn = width  if(width  is not None) else wao * (height / hao);
    fhandle.set_figheight(hn / 25.4)
    fhandle.set_figwidth( wn / 25.4)
    ahandle.set_aspect('auto')
    # adjust margins 
    if(margins is None):
        # so that we have exactly 1 pixel (to show the black border)
        hfp = fhandle.get_figheight() * dpi
        wfp = fhandle.get_figwidth() * dpi
        fhandle.subplots_adjust(left=1/wfp, right=1-1/wfp, top=1-1/hfp, bottom=1/hfp)
    elif(type(margins) is list):
        # based on user input
        fhandle.subplots_adjust(left=margins[0], right=margins[0], top=margins[0], bottom=margins[0])
    else:
        # to show everything
        fhandle.tight_layout()
...
def print_dict(dictionary,filterlist=None,printit=True):
    ...
    maxlen = len(max(list([str(key) for key in dictionary.keys()]), key=len))
    for key in keys:
        ...
        if(infilter and type(value)==list):
            if(0<len(value)<=3 and type(value[0])==int):
                text = text + str(key) + ' '*(maxlen-len(str(key)))+ ': '+str(value)+'\n'
            else:
                text = text + str(key) + ' '*(maxlen-len(str(key)))+ ': (list with '+str(len(value))+' elements)\n'
        elif(infilter and 'numpy.ndarray' in str(type(value))):
            if(len(value.shape)==1 and value.shape[0]<5):
                text = text + str(key) + ' '*(maxlen-len(str(key)))+ ': '+str(value)+'\n'
            else:
                text = text + str(key) + ' '*(maxlen-len(str(key)))+ ': (array of size '+str(value.shape)+')\n'
        elif(infilter and type(value)==dict):
            text = text + str(key) + ' '*(maxlen-len(str(key)))+ ':\n'
            ...
            text = text + str(key) + ' '*(maxlen-len(str(key)))+ ': '  + str(round(value,2)) + '\n' # scalar metrics
        elif(infilter):
