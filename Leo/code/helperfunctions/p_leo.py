...
ROOTP = '/home/olikugel/'
BASEP = '/home/olikugel/OlisIDP/Leo/'
import sys
sys.path.insert(0, BASEP + 'code/helperfunctions')
#%%
...
def load_segmentation(dataset,pid,load_partial,THR_dim=200):
    ...
    full_filepath = ROOTP + 'OlisIDP/LocalData/' + dataset + '/C02_tumorBoost/patchvolume_' + str(pid) + '.nii'
    if(os.path.isfile(full_filepath) == False):
        # print('WARNING: cannot load local patch file ' + str(full_filepath) +  ' and thus, cannot consider partial blobs properly.')
    ...
    if(load_partial):
        ...
        cancervol = filehandling.readNifti(ROOTP + 'OlisIDP/LocalData/'+dataset+'/patchvolume_' + str(pid))
        add_part_metastases = []
